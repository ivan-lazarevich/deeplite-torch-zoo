...
from deeplite_torch_zoo.src.objectdetection.datasets.road_sign_detection import RoadSignDetectionDataset
...
def _get_road_sign_for_yolo(root, num_classes=None, img_size=448):
    train_dataset = RoadSignDetectionDataset(
        root=root,
        num_classes=num_classes,
        split="train",
        img_size=img_size,
    )
    val_dataset = RoadSignDetectionDataset(
        root=root,
        num_classes=num_classes,
        split="val",
        img_size=img_size,
    )
    test_dataset = RoadSignDetectionDataset(
        root=root,
        num_classes=num_classes,
        split="test",
        img_size=img_size,
...
def get_road_sign_for_yolo(
    ...
    train_dataset, val_dataset, test_dataset = _get_road_sign_for_yolo(
        data_root, num_classes=num_classes, img_size=img_size
    )
    ...
    val_loader = get_dataloader(val_dataset, batch_size=batch_size, num_workers=num_workers, fp16=fp16,
        distributed=distributed, shuffle=False, collate_fn=val_dataset.collate_img_label_fn, device=device)
